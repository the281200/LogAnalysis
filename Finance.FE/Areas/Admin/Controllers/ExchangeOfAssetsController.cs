...
namespace WEB.Areas.Admin.Controllers
{
    ...
    public class ExchangeOfAssetsController : Controller
    {
        ...
        {
            var incurred = db.ExchangeOfAssets.AsNoTracking().Select(x => new
            {
                x.ID,
                x.IncurredDate,
        ...
        public JsonResult Get_Buyer(int? Seller)
        {
            ...
            var users = from x in db.UserProfiles.AsNoTracking() where (x.IsActive != false && x.Type == (int)TypeAccount.Customer && x.UserId != Seller) select x;
            return Json(users.Select(x => new
            {
                Id = x.UserId,
                Name = x.FullName
        ...
        public ActionResult FindInterest(BuyAndSellBond model)
        {
            ...
            return Json(new { ErrorMessage = string.Empty, Interest = interest }, JsonRequestBehavior.AllowGet);
        }
        ...
        public ActionResult CheckSellerValid(BuyAndSellBond model)
        {
            ...
            return Json(new { ErrorMessage = string.Empty, GetAsset = getAsset }, JsonRequestBehavior.AllowGet);
        }
