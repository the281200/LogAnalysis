...
namespace WEB.Areas.Admin.Controllers
{
    ...
    public class IncurredPurchasesController : BaseController
    {
        ...
        {
            var getSettlementIncurred = db.IncurredPurchases.Where(y => y.IsActive != false && y.TransactionType == 2 && y.BuyAndSellBondId != BuyAndSellBondId).Select(y => y.BuyAndSellBondId).ToList();
        ...
        public JsonResult Get_TransactionType()
        {
            ...
            new ComboBoxViewModel{
                Id = (int)TypeTransaction.GetInterest,
                Name = ((TypeTransaction)TypeTransaction.GetInterest).GetStringValue()
            },
            new ComboBoxViewModel{
                Id = (int)TypeTransaction.ContractSettlement,
                Name = ((TypeTransaction)TypeTransaction.ContractSettlement).GetStringValue()
            },
        ...
        public ActionResult Add(/*[Bind(Exclude = "ID")]*/ IncurredPurchase model)
        {
            if (ModelState.IsValid)
            {
                ...
                var getSumValueInterest = db.IncurredPurchases.Where(x => x.BuyAndSellBondId == model.BuyAndSellBondId && x.IsActive == true && x.TransactionType == (int)(TypeTransaction.GetInterest)).Select(y => y.AmountOfMoney).Sum();
                ...
                if (model.TransactionType == (int)(TypeTransaction.GetInterest) && model.AmountOfMoney + getSumValue > getValueOfAsset + getSumValueInterest)
                {
        ...
        public ActionResult CalMoney(IncurredPurchase model)
        {
            ...
            int? idInterestPaymentPeriods = null;
            ...
            else if (model.TransactionType == 1)

            {
                ...
                amountOfMoney = db.InterestPaymentPeriods.Where(x => x.IsActive != false && x.BuyAndSellBondId == model.BuyAndSellBondId && x.InterestPaymentDate == getInterestPaymentDate).Select(y => y.AccruedInterest).FirstOrDefault();
                idInterestPaymentPeriods = db.InterestPaymentPeriods.Where(x => x.AccruedInterest == amountOfMoney && x.BuyAndSellBondId == model.BuyAndSellBondId && x.InterestPaymentDate == getInterestPaymentDate).Select(y => y.Id).FirstOrDefault();
                 stringAmountOfMoney = String.Format(new CultureInfo("en-US"), "{0:#,##0.##}", amountOfMoney);
            }
            ...
            return Json(new { ErrorMessage = string.Empty, AmountOfMoney = stringAmountOfMoney, ContactDisbursementId = idDisbursement, InterestPaymentPeriodsId = idInterestPaymentPeriods }, JsonRequestBehavior.AllowGet);
        }
