...
namespace WEB.Controllers
{
    public class AccountController : Controller
    {
        ...
        public bool SendMailLinkForgot(ConfigSendMailInfo configSendMailInfo, string link, UserProfile user)
        {
            ...
            try
            {
                var getFeSite = db.WebConfigs.Where(x => x.Key == "SiteFE").Select(x => x.Value).FirstOrDefault().ToString();
                ...
                message.Body += "<img src = '" + getFeSite + "/Content/themes/admin/img/footerMail.jpg' style='max-width:650px' width='100%' />";
                message.Body += "</td>";
                message.Body += "<td style='width:20px' width='20'>";
                message.Body += "<div lang='space40'>";
                message.Body += "</div>";
        ...
        public JsonResult LoginPost(LoginCustomerModel model)
        {
            try
            {
                ...
                var checkUser = db.UserProfiles.Where(x => x.UserName == model.UserName && x.Type != (int)TypeAccount.Admin).Any();
