...
namespace WEB.Controllers
{
    public class HomeController : BaseController
    {
        ...
        //}
        public int TodayWarning(int? userid)
        {
            /*var userid = WebSecurity.CurrentUserId;*/
            var dateCompare = DateTime.Now.AddDays(6);
            var last5days = DateTime.Today.AddDays(5);
            var getInterestPaymentPeriodsId = db.IncurredPurchases.Where(x => x.InterestPaymentPeriodsId != null && x.CustomerId == userid ).Select(x => x.InterestPaymentPeriodsId).ToList();
            ...
            //last 5 days
            var getNumEarningDueWarningLast5Days = db.InterestPaymentPeriods.Where(y => (!getInterestPaymentPeriodsId.Contains(y.Id)) && y.BuyAndSellBond.CustomerId == userid && y.BuyAndSellBondId != 0 && y.IsActive != false && y.InterestPaymentDate != null && DbFunctions.TruncateTime(y.InterestPaymentDate) == last5days).Count();

            var getNumSettlementWarningLast5Days = db.IncurredPurchases.Where(y => (y.TransactionType == (int)(TypeTransaction.ContractSettlementByTime) || y.TransactionType == (int)(TypeTransaction.ContractSettlementOnTime)) && y.CustomerId == userid && y.IsActive != false && y.IncurredDate != null && DbFunctions.TruncateTime(y.IncurredDate) == last5days).Count();
        ...
        public ActionResult RequestCheckNoti()
        {
            ...
            var userCustomer = db.UserProfiles.Where(x => x.IsActive != false && x.Type != (int)TypeAccount.Admin).ToList();

            foreach (var item in userCustomer)
            {
                var sumWarning = TodayWarning(item.UserId);
