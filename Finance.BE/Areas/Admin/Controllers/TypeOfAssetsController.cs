    ...
    public class TypeOfAssetsController : BaseController
    {
        ...
        public ActionResult Add(/*[Bind(Exclude = "ID")]*/ TypeOfAsset model)
        {
            if (ModelState.IsValid)
            {
                ...
                var assetId = (from p in db.Set<TypeOfAsset>().AsNoTracking()
                               where p.AssetId.Equals(model.AssetId, StringComparison.OrdinalIgnoreCase) && p.IsActive != false
                ...
                else
                {
                    var assetName = (from p in db.Set<TypeOfAsset>().AsNoTracking()
                                     where p.AssetName.Equals(model.AssetName, StringComparison.OrdinalIgnoreCase) && p.IsActive != false
                                     select p).FirstOrDefault();
                    if (assetName != null)
                    {
                    ...
                    else
                    {
                        ...
                        {
                            ActiveType = LogConstant.AddTypeOfAsset,
                            FunctionName = LogConstant.ManageTypeOfAsset,
                            Information = model.ToJson(),
                            DataTable = LogConstant.TypeOfAsset,
        ...
        public ActionResult Edit(TypeOfAsset model, string assetId, string assetName)
        {
            if (ModelState.IsValid)
            {
                ...
                var checkAssetIdExist = asset.Where(p => p.ID != model.ID && p.AssetId == model.AssetId && p.IsActive != false).Any();
                ...
                var checkAssetNameExist = asset.Where(p => p.ID != model.ID && p.AssetName.Equals(model.AssetName, StringComparison.OrdinalIgnoreCase) && p.IsActive != false).Any();
                ...
                AuditLog log = new AuditLog
                {
                    ActiveType = LogConstant.EditTypeOfAsset,
                    FunctionName = LogConstant.ManageTypeOfAsset,
                    Information = model.ToJson(),
                    DataTable = LogConstant.TypeOfAsset,
                    UserName = userCurent.FullName + "(" + userCurent.UserName + ")",
        ...
        public JsonResult Delete(int id)
        {
            try
            {
                ...
                AuditLog log = new AuditLog
                {
                    ActiveType = LogConstant.DeleteTypeOfAsset,
                    FunctionName = LogConstant.ManageTypeOfAsset,
        ...
        public JsonResult Undo(string data)
        {
            ...
            try
            {
                ...
                AuditLog log = new AuditLog
                {
                    ActiveType = LogConstant.UndoTypeOfAsset,
                    FunctionName = LogConstant.ManageTypeOfAsset,
