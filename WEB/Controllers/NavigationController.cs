...
namespace WEB.Controllers
{
    public class NavigationController : BaseController
    {
        ...
        [ChildActionOnly]
        //  [OutputCache(Duration = 60, VaryByCustom = "culture")]
        public ActionResult _MainUser(string key)
        {
            var temp = db.ModuleNavigations.AsNoTracking().Where(
                x => x.Navigation.Key.ToLower().Equals(key.ToLower())).Select(x => x.WebModuleID);
            //var webmodules = from x in db.WebModules
            ...
            //                 orderby x.Order
            //                 select x;
            var model = db.WebModules.Include("SubWebModules").Where(x => temp.Contains(x.ID)).OrderByDescending(x => x.ModifiedDate).ToList();
            var temm = model.ToList();
